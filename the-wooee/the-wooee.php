 ...
 * ====================================== */
// enqueue wooee lightbox bg
function the_wooee_enqueue_custom_css() {
  wp_enqueue_style( 'wooee-lightbox-bg', plugin_dir_url( __FILE__ ) . 'assets/css/wooee-lightbox-bg.css', array('photoswipe'), '1.0' );
}
add_action( 'wp_enqueue_scripts', 'the_wooee_enqueue_custom_css' );
// enqueue wooee lightbox bg end
add_action('wp_enqueue_scripts', function () {
  ...
  // 8) Product pages styles
  if (the_wooee_opt('enable_custom_shop') && is_product_category()) {
    $rel = 'assets/css/product-pages.css';
    $file = plugin_dir_path(__FILE__) . $rel;
    if (file_exists($file)) {
      $url = plugins_url($rel, __FILE__);
      $ver = (string) filemtime($file);
      wp_enqueue_style('the-wooee-product-pages', $url, $deps, $ver);
    }
  }
}, 999);
...
/**
 * Remove the redundant "Description" heading inside the product tabs
 * (keeps the tab itself, removes the big <h2>Description</h2> right above your text)
 */
