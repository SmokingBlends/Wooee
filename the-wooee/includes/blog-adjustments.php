...
function the_wooee_use_excerpt_on_blog_pages_with_read_more( $content ) {
    if ( is_home() || is_archive() ) {
        global $post;
        // Temporarily remove this filter to prevent recursion when calling get_the_excerpt()
        remove_filter( 'the_content', 'the_wooee_use_excerpt_on_blog_pages_with_read_more' );
        $excerpt = get_the_excerpt();
        // Add the filter back
        add_filter( 'the_content', 'the_wooee_use_excerpt_on_blog_pages_with_read_more' );
...
if ( the_wooee_opt( 'blog_excerpts_enabled' ) ) {
    add_action( 'init', 'the_wooee_blog_excerpts_custom_init' );
}

function the_wooee_blog_excerpts_custom_init() {
...
function the_wooee_add_blog_page_header() {
...
add_action( 'storefront_loop_before', 'the_wooee_add_blog_page_header' );
