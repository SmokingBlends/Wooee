...
add_action('woocommerce_new_product', 'wee_premium_clear_product_transient');
add_action('woocommerce_update_product', 'wee_premium_clear_product_transient');
add_action('before_delete_post', 'wee_premium_clear_product_transient_on_delete');
function wee_premium_clear_product_transient($product_id) {
    delete_transient('wee_premium_review_products');
}
function wee_premium_clear_product_transient_on_delete($post_id) {
...
add_shortcode('submit_review_form', 'wee_premium_review_form_shortcode');
function wee_premium_review_form_shortcode() {
    ...
    if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_review'])) {
        ...
        if (!isset($_POST['_wpnonce'])) {
            wc_add_notice(__('Security check failed. Please try again.', 'wee-premium'), 'error');
        ...
        if (!empty($_POST['hp_email'])) {
            wc_add_notice(__('Error submitting review. Please try again.', 'wee-premium'), 'error'); // Generic error to not tip off bots
            wp_safe_redirect($current_url);
            exit;
        }
        ...
        if (!$product_id || !$review_content || (!$email && !is_user_logged_in()) || (wc_review_ratings_enabled() && !$rating)) {
            wc_add_notice(__('Please fill out all required fields.', 'wee-premium'), 'error');
            wp_safe_redirect($current_url);
            exit;
        }
        if (wc_review_ratings_enabled() && ($rating < 1 || $rating > 5)) {
            wc_add_notice(__('Rating must be between 1 and 5.', 'wee-premium'), 'error');
            wp_safe_redirect($current_url);
            exit;
        }
        if ($email && !is_email($email)) {
            wc_add_notice(__('Please provide a valid email address.', 'wee-premium'), 'error');
            wp_safe_redirect($current_url);
            exit;
        }
        ...
        if (!empty($existing_reviews)) {
            wc_add_notice(__('You have already reviewed this product.', 'wee-premium'), 'error');
            wp_safe_redirect($current_url);
            exit;
        }
        ...
        $use_turnstile = wee_premium_opt('enable_turnstile') && is_plugin_active('login-security-pro/login-security-pro.php');
        if ($use_turnstile) {
        ...
        if (!$comment_id) {
            wc_add_notice(__('Error submitting review. Please try again.', 'wee-premium'), 'error');
            wp_safe_redirect($current_url);
            exit;
        }
        if ($rating) {
        ...
        $notice = (1 === $comment_approved) ? __('Review published successfully!', 'wee-premium') : __('Review submitted successfully! It will be reviewed before publishing.', 'wee-premium');
    ...
    return wee_premium_render_review_form();
}
// Render the review form
function wee_premium_render_review_form() {
    ob_start();
    global $post;
    $request_uri = isset($_SERVER['REQUEST_URI']) ? sanitize_text_field(wp_unslash($_SERVER['REQUEST_URI'])) : '/';
    $current_url = is_a($post, 'WP_Post') ? get_permalink($post->ID) : home_url($request_uri);
    echo '<div class="woocommerce"><div id="reviews" class="woocommerce-Reviews">';
    echo '<h2 class="woocommerce-Reviews-title">' . esc_html__('Leave a Review', 'wee-premium') . '</h2>';
    ...
    <p class="comment-form-product form-row form-row-wide">
        <label for="product_id"><?php esc_html_e('Select Product', 'wee-premium'); ?> <span class="required">*</span></label>
        <select name="product_id" id="product_id" class="input-text" required>
            <option value=""><?php esc_html_e('Choose a product...', 'wee-premium'); ?></option>
            <?php
            $transient_key = 'wee_premium_review_products';
            ...
            if (empty($products)) {
                echo '<option value="" disabled>' . esc_html__('No products available', 'wee-premium') . '</option>';
            } else {
    ...
    if (!is_user_logged_in()) {
        ...
        <p class="comment-form-author form-row form-row-first">
            <label for="author"><?php esc_html_e('Name', 'wee-premium'); ?><?php echo $req ? ' <span class="required">*</span>' : ''; ?></label>
        ...
        <p class="comment-form-email form-row form-row-last">
            <label for="email"><?php esc_html_e('Email', 'wee-premium'); ?> <span class="required">*</span></label>
    ...
    if (wc_review_ratings_enabled()) {
        ...
        <p class="comment-form-rating form-row form-row-wide">
            <label for="rating"><?php esc_html_e('Your rating', 'wee-premium'); ?> <span class="required">*</span></label>
            <select name="rating" id="rating" required>
                <option value=""><?php esc_html_e('Rate&hellip;', 'wee-premium'); ?></option>
                <option value="5"><?php esc_html_e('Perfect', 'wee-premium'); ?></option>
                <option value="4"><?php esc_html_e('Good', 'wee-premium'); ?></option>
                <option value="3"><?php esc_html_e('Average', 'wee-premium'); ?></option>
                <option value="2"><?php esc_html_e('Not that bad', 'wee-premium'); ?></option>
                <option value="1"><?php esc_html_e('Very poor', 'wee-premium'); ?></option>
            </select>
        </p>
        <?php
    }
    ...
    <p class="comment-form-comment form-row form-row-wide">
        <label for="comment"><?php esc_html_e('Your review', 'wee-premium'); ?> <span class="required">*</span></label>
        <textarea id="comment" name="comment" cols="45" rows="8" class="input-text" required></textarea>
    </p>
    <p class="form-submit">
        <input name="submit_review" type="submit" id="submit" class="submit button" value="<?php esc_attr_e('Submit Review', 'wee-premium'); ?>" />
    </p>
    </form>
    </div>
    </div>
...
add_shortcode('display_all_reviews', 'wee_premium_display_all_reviews_shortcode');
function wee_premium_display_all_reviews_shortcode() {
    $reviews = wee_premium_opt('reviews');
    if (empty($reviews)) {
        return '';
    }
    ob_start();
    echo '<div class="woocommerce"><div id="reviews" class="woocommerce-Reviews">';
    echo '<h2 class="woocommerce-Reviews-title">' . esc_html__('Customer Reviews', 'wee-premium') . '</h2>';
            ...
            <div class="woocommerce-review__container">
                ...
                <?php if (wc_review_ratings_enabled()) : ?>
                    <div class="woocommerce-review__rating rating">
                        <div class="star-rating" role="img" aria-label="<?php echo esc_attr(sprintf(/* translators: %s: rating */ __('%s out of 5', 'wee-premium'), $review['rating'])); ?>">
