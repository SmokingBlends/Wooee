...
function the_wooee_item_specifics_render_meta_box( $post ) {
    ...
    echo '<p class="description">' . esc_html__( 'One per line: Label: Value. Lists comma-separated. Basic HTML allowed in values. Lines without colon are headings.', 'the-wooee' ) . '</p>';
}
...
function the_wooee_item_specifics_inject_table( $product ) {
    ...
    foreach ( $pairs as $pair ) {
        if ( $pair['type'] === 'heading' ) {
            echo '<tr class="heading"><td colspan="2" style="text-align: center; background: #f0f0f0; padding: .3em 0;"><h4 style="margin: 0;">' . wp_kses_post( $pair['value'] ) . '</h4></td></tr>';
        } else {
...
function the_wooee_item_specifics_parse_label_value_lines( string $content ) : array {
    ...
    foreach ( $lines as $line ) {
        ...
        if ( count( $parts ) === 1 ) {
            ...
            $value = trim( $parts[0] );
            if ( $value !== '' ) {
                $out[] = [ 'type' => 'heading', 'value' => $value ];
            }
        } elseif ( count( $parts ) === 2 ) {
            $label = trim( $parts[0] );
            $value = trim( $parts[1] );
            if ( $label !== '' && $value !== '' ) {
                $out[] = [ 'type' => 'pair', 'label' => $label, 'value' => $value ];
            }
        }
    }
    return $out;
