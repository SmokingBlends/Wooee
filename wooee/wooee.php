<?php
/*
Plugin Name: Wooee! Icing on Your WooCommerce Cake & a Sprinkle of Storefront
...
define('WOOEE_VER', '0.3.1');
define('WOOEE_PLUGIN_FILE', __FILE__);
/** ==============================
 * Options (defaults + helpers)
 * ============================== */
const WOOEE_DEFAULTS = [
...
function wooee_opts() {
  $saved = get_option('wooee_options');
  if (!is_array($saved)) $saved = [];
  return array_merge(WOOEE_DEFAULTS, $saved);
}
function wooee_opt($key) {
  $o = wooee_opts();
  return isset($o[$key]) ? $o[$key] : (WOOEE_DEFAULTS[$key] ?? null);
}
// Include the blog adjustments file conditionally
$wooee_theme = wp_get_theme();
$wooee_is_storefront = ( 'storefront' === $wooee_theme->get_stylesheet() || 'storefront' === $wooee_theme->get_template() );
if (wooee_opt('blog_excerpts_enabled') && $wooee_is_storefront) {
...
if (wooee_opt('enable_facebook_cart')) {
...
add_action('after_setup_theme', function() {
    global $wooee_is_storefront;
    if ($wooee_is_storefront) {
        remove_action('storefront_footer', 'storefront_credit', 20);
        add_action('storefront_footer', 'wooee_custom_storefront_credit', 20);
    }
});
function wooee_custom_storefront_credit() {
    $credit_text = wooee_opt('footer_credit_text');
    $credit_url = wooee_opt('footer_credit_url');
    $links_output = '';
...
add_action('wp_enqueue_scripts', 'wooee_enqueue_wc_styles_for_review_shortcode');
function wooee_enqueue_wc_styles_for_review_shortcode() {
    global $post;
...
add_action('wp_enqueue_scripts', function () {
  ...
  if (wooee_opt('load_links_css')) {
    $rel  = 'assets/css/links.css';
    $file = plugin_dir_path(__FILE__) . $rel;
    $url  = plugins_url($rel, __FILE__);
    $ver  = file_exists($file) ? (string) filemtime($file) : WOOEE_VER;
    wp_enqueue_style('wooee-links', $url, $deps, $ver);
  }
  // 2) Hover highlight on product & category boxes
  if (wooee_opt('load_lift_css')) {
    $rel  = 'assets/css/lift.css';
    $file = plugin_dir_path(__FILE__) . $rel;
    $url  = plugins_url($rel, __FILE__);
    $ver  = file_exists($file) ? (string) filemtime($file) : WOOEE_VER;
    wp_enqueue_style('wooee-lift', $url, $deps, $ver);
  }
  // 3) Keyboard-only focus rings (CSS only)
  if (wooee_opt('keyboard_only_focus')) {
    $rel  = 'assets/css/focus.css';
    $file = plugin_dir_path(__FILE__) . $rel;
    $url  = plugins_url($rel, __FILE__);
    $ver  = file_exists($file) ? (string) filemtime($file) : WOOEE_VER;
    wp_enqueue_style('wooee-focus', $url, [], $ver);
  }
  // 4) Blog excerpts "Read More" button (new feature)
  if (wooee_opt('blog_excerpts_enabled') && (is_home() || is_archive())) {
    $rel  = 'assets/css/snippet-articles.css';
    $file = plugin_dir_path(__FILE__) . $rel;
    $url  = plugins_url($rel, __FILE__);
    $ver  = file_exists($file) ? (string) filemtime($file) : WOOEE_VER;
    wp_enqueue_style('wooee-blog-excerpts', $url, $deps, $ver);
  }
  ...
  if (wooee_opt('enable_reviews')) {
    ...
    if (file_exists($file)) {
      ...
      wp_enqueue_style('wooee-reviews', $url, $deps, $ver);
    }
  }
  // 6) Item specifics
  ...
  if (file_exists($file)) {
      $url = plugins_url($rel, __FILE__);
      $ver = (string) filemtime($file);
      wp_enqueue_style('wooee-item-specifics', $url, $deps, $ver);
  }
}, 999);
// Create secure-checkout page on activation if not exists
register_activation_hook(__FILE__, 'wooee_create_secure_checkout_page');
function wooee_create_secure_checkout_page() {
  $slug = wooee_opt('facebook_cart_page_slug');
...
register_activation_hook(__FILE__, 'wooee_create_reviews_page');
function wooee_create_reviews_page() {
  $slug = wooee_opt('reviews_page_slug');
  if ($slug && !get_page_by_path($slug)) {
    $page = [
      'post_title'   => 'Reviews',
      'post_name'    => $slug,
...
if (wooee_opt('enable_item_specifics')) {
    add_filter('woocommerce_product_tabs', function($tabs) {
        if (isset($tabs['additional_information'])) {
            $tabs['additional_information']['title'] = __('Item Specifics', 'wooee');
        }
        return $tabs;
    }, 20);
    add_filter( 'woocommerce_product_additional_information_heading', function() { return __('Item Specifics', 'wooee'); } );
}
// Enqueue admin CSS on settings page
add_action('admin_enqueue_scripts', 'wooee_admin_scripts');
function wooee_admin_scripts($hook) {
  if ($hook !== 'settings_page_wooee') return;
  wp_enqueue_style('wooee-admin', plugins_url('assets/css/admin.css', __FILE__), [], WOOEE_VER);
}