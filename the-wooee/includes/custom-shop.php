...
// 1. Clear the main product query for the shop page ONLY (not search results)
add_action( 'woocommerce_product_query', function( $query ) {
    if ( is_shop() && ! is_search() ) {   // ← FIXED
        $query->set( 'post__in', array(0) );
    }
} );

// 2. Disable default category/product displays from settings ONLY on real shop page
add_filter( 'option_woocommerce_shop_page_display', function( $value ) {
    return ( is_shop() && ! is_search() ) ? '' : $value;   // ← FIXED
} );
...
add_action( 'wp', function() {
    if ( ( is_shop() && ! is_search() ) || is_product_category() ) {   // ← FIXED
...
/**
 * RENDERING CUSTOM CATEGORY ROWS — ONLY on the real shop page
 */
add_action( 'woocommerce_archive_description', function() {
    if ( ! ( is_shop() && ! is_search() ) ) return;   // ← FIXED (this was the main culprit)
