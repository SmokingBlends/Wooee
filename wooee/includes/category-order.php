...
add_action('product_cat_add_form_fields', 'wooee_category_order_add_field');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function wooee_category_order_add_field() {
    wp_nonce_field('wooee_category_order', 'wooee_category_order_nonce');
    ?>
...
add_action('product_cat_edit_form_fields', 'wooee_category_order_edit_field', 10, 2);
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function wooee_category_order_edit_field($term, $taxonomy) {
    wp_nonce_field('wooee_category_order', 'wooee_category_order_nonce');
...
// Save the field
add_action('created_product_cat', 'wooee_category_order_save');
add_action('edited_product_cat', 'wooee_category_order_save');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function wooee_category_order_save($term_id) {
    $nonce = isset($_POST['wooee_category_order_nonce']) ? sanitize_text_field(wp_unslash($_POST['wooee_category_order_nonce'])) : '';
    if (!$nonce || !wp_verify_nonce($nonce, 'wooee_category_order')) {
        return;
    }
...
add_filter('woocommerce_product_subcategories_args', 'wooee_category_order_args');
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function wooee_category_order_args($args) {
    $args['wooee_category_order'] = true;
...
add_filter('get_terms', 'wooee_sort_product_categories', 10, 3);
// phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedFunctionFound
function wooee_sort_product_categories($terms, $taxonomies, $args) {
    if (!in_array('product_cat', (array) $taxonomies) || empty($args['wooee_category_order'])) {
        return $terms;
    }
